//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_import

import 'package:one_of_serializer/any_of_serializer.dart';
import 'package:one_of_serializer/one_of_serializer.dart';
import 'package:built_collection/built_collection.dart';
import 'package:built_value/json_object.dart';
import 'package:built_value/serializer.dart';
import 'package:built_value/standard_json_plugin.dart';
import 'package:built_value/iso_8601_date_time_serializer.dart';
import 'package:moosyl/src/date_serializer.dart';
import 'package:moosyl/src/model/date.dart';

import 'package:moosyl/src/model/checkout_session_create.dart';
import 'package:moosyl/src/model/checkout_session_create_body.dart';
import 'package:moosyl/src/model/checkout_session_create_data.dart';
import 'package:moosyl/src/model/checkout_session_get.dart';
import 'package:moosyl/src/model/checkout_session_get_environment.dart';
import 'package:moosyl/src/model/checkout_session_get_latest_payment.dart';
import 'package:moosyl/src/model/checkout_session_pay_body.dart';
import 'package:moosyl/src/model/checkout_session_select_method_body.dart';
import 'package:moosyl/src/model/checkout_session_switch_method.dart';
import 'package:moosyl/src/model/configuration_create.dart';
import 'package:moosyl/src/model/configuration_create_data.dart';
import 'package:moosyl/src/model/configuration_get.dart';
import 'package:moosyl/src/model/configuration_get_data.dart';
import 'package:moosyl/src/model/configuration_list.dart';
import 'package:moosyl/src/model/configuration_list_data_inner.dart';
import 'package:moosyl/src/model/cron_result.dart';
import 'package:moosyl/src/model/customer_create.dart';
import 'package:moosyl/src/model/customer_get.dart';
import 'package:moosyl/src/model/customer_get_data.dart';
import 'package:moosyl/src/model/customer_list.dart';
import 'package:moosyl/src/model/customer_update.dart';
import 'package:moosyl/src/model/get_checkout_session_public_by_id_payment_status200_response.dart';
import 'package:moosyl/src/model/get_organization_by_id200_response.dart';
import 'package:moosyl/src/model/get_organization_by_id200_response_data.dart';
import 'package:moosyl/src/model/get_payment_by_id_status200_response.dart';
import 'package:moosyl/src/model/get_products_page_parameter.dart';
import 'package:moosyl/src/model/invoice_get.dart';
import 'package:moosyl/src/model/invoice_get_data.dart';
import 'package:moosyl/src/model/invoice_list.dart';
import 'package:moosyl/src/model/payment_create.dart';
import 'package:moosyl/src/model/payment_get.dart';
import 'package:moosyl/src/model/payment_get_data.dart';
import 'package:moosyl/src/model/payment_request_create.dart';
import 'package:moosyl/src/model/payment_request_get.dart';
import 'package:moosyl/src/model/payment_request_get_data.dart';
import 'package:moosyl/src/model/payment_request_refresh_status.dart';
import 'package:moosyl/src/model/payment_request_refresh_status_banking_api_responses_inner.dart';
import 'package:moosyl/src/model/payment_update.dart';
import 'package:moosyl/src/model/post_checkout_session_public_by_id_pay200_response.dart';
import 'package:moosyl/src/model/post_checkout_session_public_by_id_simulate_transfer_request.dart';
import 'package:moosyl/src/model/post_connect_exchange200_response.dart';
import 'package:moosyl/src/model/post_connect_exchange200_response_data.dart';
import 'package:moosyl/src/model/post_connect_exchange_request.dart';
import 'package:moosyl/src/model/post_connect_revoke200_response.dart';
import 'package:moosyl/src/model/post_connect_revoke_request.dart';
import 'package:moosyl/src/model/post_masrivi_initiate200_response.dart';
import 'package:moosyl/src/model/post_masrivi_initiate_request.dart';
import 'package:moosyl/src/model/post_payment200_response.dart';
import 'package:moosyl/src/model/post_payment_by_id_simulate_transfer200_response.dart';
import 'package:moosyl/src/model/price_create.dart';
import 'package:moosyl/src/model/price_get.dart';
import 'package:moosyl/src/model/price_update.dart';
import 'package:moosyl/src/model/price_update_amount.dart';
import 'package:moosyl/src/model/product_create.dart';
import 'package:moosyl/src/model/product_get.dart';
import 'package:moosyl/src/model/product_get_data.dart';
import 'package:moosyl/src/model/product_get_with_prices.dart';
import 'package:moosyl/src/model/product_get_with_prices_data.dart';
import 'package:moosyl/src/model/product_get_with_prices_data_prices_inner.dart';
import 'package:moosyl/src/model/product_list.dart';
import 'package:moosyl/src/model/product_list_pagination.dart';
import 'package:moosyl/src/model/product_update.dart';
import 'package:moosyl/src/model/subscription_create.dart';
import 'package:moosyl/src/model/subscription_create_by_external_user.dart';
import 'package:moosyl/src/model/subscription_create_trial.dart';
import 'package:moosyl/src/model/subscription_create_trial_end.dart';
import 'package:moosyl/src/model/subscription_get.dart';
import 'package:moosyl/src/model/subscription_get_data.dart';
import 'package:moosyl/src/model/subscription_list.dart';
import 'package:moosyl/src/model/subscription_update.dart';
import 'package:moosyl/src/model/success.dart';

part 'serializers.g.dart';

@SerializersFor([
  CheckoutSessionCreate,
  CheckoutSessionCreateBody,
  CheckoutSessionCreateData,
  CheckoutSessionGet,
  CheckoutSessionGetEnvironment,
  CheckoutSessionGetLatestPayment,
  CheckoutSessionPayBody,
  CheckoutSessionSelectMethodBody,
  CheckoutSessionSwitchMethod,
  ConfigurationCreate,
  ConfigurationCreateData,
  ConfigurationGet,
  ConfigurationGetData,
  ConfigurationList,
  ConfigurationListDataInner,
  CronResult,
  CustomerCreate,
  CustomerGet,
  CustomerGetData,
  CustomerList,
  CustomerUpdate,
  GetCheckoutSessionPublicByIdPaymentStatus200Response,
  GetOrganizationById200Response,
  GetOrganizationById200ResponseData,
  GetPaymentByIdStatus200Response,
  GetProductsPageParameter,
  InvoiceGet,
  InvoiceGetData,
  InvoiceList,
  PaymentCreate,
  PaymentGet,
  PaymentGetData,
  PaymentRequestCreate,
  PaymentRequestGet,
  PaymentRequestGetData,
  PaymentRequestRefreshStatus,
  PaymentRequestRefreshStatusBankingApiResponsesInner,
  PaymentUpdate,
  PostCheckoutSessionPublicByIdPay200Response,
  PostCheckoutSessionPublicByIdSimulateTransferRequest,
  PostConnectExchange200Response,
  PostConnectExchange200ResponseData,
  PostConnectExchangeRequest,
  PostConnectRevoke200Response,
  PostConnectRevokeRequest,
  PostMasriviInitiate200Response,
  PostMasriviInitiateRequest,
  PostPayment200Response,
  PostPaymentByIdSimulateTransfer200Response,
  PriceCreate,
  PriceGet,
  PriceUpdate,
  PriceUpdateAmount,
  ProductCreate,
  ProductGet,
  ProductGetData,
  ProductGetWithPrices,
  ProductGetWithPricesData,
  ProductGetWithPricesDataPricesInner,
  ProductList,
  ProductListPagination,
  ProductUpdate,
  SubscriptionCreate,
  SubscriptionCreateByExternalUser,
  SubscriptionCreateTrial,
  SubscriptionCreateTrialEnd,
  SubscriptionGet,
  SubscriptionGetData,
  SubscriptionList,
  SubscriptionUpdate,
  Success,
])
Serializers serializers = (_$serializers.toBuilder()
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ProductGetWithPricesData)]),
        () => ListBuilder<ProductGetWithPricesData>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(SubscriptionGetData)]),
        () => ListBuilder<SubscriptionGetData>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ConfigurationListDataInner)]),
        () => ListBuilder<ConfigurationListDataInner>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(CustomerGetData)]),
        () => ListBuilder<CustomerGetData>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(PaymentRequestRefreshStatusBankingApiResponsesInner)]),
        () => ListBuilder<PaymentRequestRefreshStatusBankingApiResponsesInner>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(InvoiceGetData)]),
        () => ListBuilder<InvoiceGetData>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ProductGetWithPricesDataPricesInner)]),
        () => ListBuilder<ProductGetWithPricesDataPricesInner>(),
      )
      ..add(const OneOfSerializer())
      ..add(const AnyOfSerializer())
      ..add(const DateSerializer())
      ..add(Iso8601DateTimeSerializer())
    ).build();

Serializers standardSerializers =
    (serializers.toBuilder()..addPlugin(StandardJsonPlugin())).build();
